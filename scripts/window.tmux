...
if-shell -b '[[ $(echo "$TMUX_VERSION >= 2.9" | bc) -eq 1 ]]' " \
	set-option -g pane-border-style $tmux_black \; \
	set-option -g pane-active-border-style $tmux_black"
if-shell '[[ $(echo "$TMUX_VERSION < 2.9" | bc) -eq 1 ]]' " \
	set-option -g pane-border-bg $tmux_black \; \
	set-option -g pane-active-border-bg $tmux_black"
